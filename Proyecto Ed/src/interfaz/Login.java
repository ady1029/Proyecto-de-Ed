...
public class Login extends JFrame {
	...
	private static RedSystem system1;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					system1= new RedSystem("WorkSphere");
					inicializarDatos(system1);
	...
	public Login() {
		...
		btnAccederLogin.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try{
					if(accederAdm()== true){
						MenuPrincAdmin frame = new MenuPrincAdmin(system1);
					...
					if(p != null){
						MenuUsuario frame = new MenuUsuario(Login.this,system1,p);
						frame.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
						frame.setVisible(true);
						setVisible(false);
						counter--;
		...
		btnRegistrarse.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				CrearCuenta dialog = new CrearCuenta(system1);
	...
	public Person acceder(Person p) {
		p = null;
		LinkedList <Vertex> lista = system1.getGraph().getVerticesList();
		Iterator<Vertex> it = lista.iterator();
		Vertex temp;
		while(it.hasNext() && p == null) {
			temp = it.next();
			Person current = (Person)temp.getInfo();
			if(current.getNick().equalsIgnoreCase(textFieldNombreUsuario.getText()) && current.getPassword().equalsIgnoreCase(passwordField.toString())) {
				p = current;
			}
		}
		return p;
	...
	public boolean accederAdm() {
		boolean a = false;
		if(system1.getAdm().getNick().equalsIgnoreCase(textFieldNombreUsuario.getText()) && system1.getAdm().getPassword().equalsIgnoreCase(passwordField.toString())) {
			a = true;
		}
		return a;
	}
	public static void inicializarDatos(RedSystem system1) {
