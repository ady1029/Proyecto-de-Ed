...
public class PanelBuscar extends JDialog {
	...
	public PanelBuscar(SolicitudesOpciones father, RedSystem sistema, Person p) {
		super(father, true);
		red = sistema;
		setTitle("Buscar Usuario");
		setDefaultCloseOperation(JDialog.DO_NOTHING_ON_CLOSE);
		...
		btnmcnBuscar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(!a.equalsIgnoreCase("")) {
					Vertex aux= sistema.findNick(a);
					Vertex aux1= sistema.findNick(p.getNick());
					if((aux !=null) && (!aux1.getAdjacents().contains(aux)) && aux1 != aux) {
						try{
							InfoUsuario dialog = new InfoUsuario(father, (Person)aux.getInfo(),sistema,p, true );
							dialog.setDefaultCloseOperation(dialog.DISPOSE_ON_CLOSE);
							dialog.setVisible(true);
							dispose();
						...
						JOptionPane.showMessageDialog(PanelBuscar.this, "No se encuentra el usuario introducido en la lista de usuarios que no son sus amigos.");
